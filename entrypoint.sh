...
iptables -t nat -I PREROUTING -d 172.17.0.1/32 -p tcp -m tcp --dport 80 -j DNAT --to-destination $(/usr/bin/awk ' /lemp/ { print $1 } ' /etc/hosts):80
iptables -t nat -I PREROUTING -d 172.17.0.1/32 -p tcp -m tcp --dport 443 -j DNAT --to-destination $(/usr/bin/awk ' /lemp/ { print $1 } ' /etc/hosts):443
